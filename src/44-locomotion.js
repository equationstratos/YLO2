...
(function (Y) {
  ...
  /* Ce qu'une ROUE avale sans qu'une patte s'en mêle, sur l'instant.
     `WHEEL_CLIMB` dit jusqu'où une patte peut aller CHERCHER une prise ; il ne
     dit rien de l'endroit où la roue se trouve maintenant. On s'en servait
     pourtant pour autoriser l'avance, et la roue traversait alors la paroi
     verticale d'un quarter pipe — 334 mm de pneu dans le béton, le temps que
     la suspension rattrape les 450 mm. L'avance se juge donc sur la roue :
     tant qu'elle n'est pas montée au niveau, la caisse ne passe pas. Le robot
  ...
  let spinTurn = 1;
  /* Vrille CONSERVÉE dans une tenue : le robot passe de la pirouette au
     cabrage ou à l'équilibre sans cesser de tourner. Plafonnée plus bas que
     la pirouette libre — on ne tient pas dix radians par seconde en équilibre
     sur deux roues — et freinée dès qu'on lâche. */
  const TWIRL_MAX = 5.0;            // rad/s tenus en tenue
  const TWIRL_RAMP = 7.0;           // rad/s² de mise en route et de freinage
  /* Saut sur place depuis une tenue : une détente courte, verticale, qui ne
     change ni l'assiette ni l'appui — le robot saute DANS sa position. */
  const HOP_VZ = 1.75;
  const HOP_CHARGE = 0.45;          // s d'armement pour une détente pleine
  const HOP_CROUCH = 0.055;         // m de ramassé pendant l'armement
  ...
  const nat = {
    ...
    vx: 0, vy: 0, wz: 0,
    ax: 0,
    plant: {}, lift: {}, land: {}, clear: {}, prevFoot: {}, wheelZ: {}, wstep: {},
    wfloor: {}, figAxle: {},
    spin: {},                              // angle de roue
    jit: {}, lastPh: {},
    auto: true,
  ...
  function stepWheels(dt, state) {
    ...
    if (!nat.wheelAir && wallBlocks(state, state.px + stepX, state.py + stepY, WHEEL_ROLL)) {
    ...
    /* Sauf sous un linteau : là on ne se redresse pas, on baisse la tête. Le
       redressement sur relief sert à garder du débattement ; sous une fenêtre
       il coûte les 30 mm qui séparent passer de taper — et depuis que
       franchir l'appui se fait vraiment en levant la patte, le sol y compte
       comme accidenté et la caisse se redressait juste sous le linteau. */
    let headroom = Infinity;
    if (Y.Terrain && Y.Terrain.ceilingAt) {
      headroom = Y.Terrain.ceilingAt(state.px, state.py, state.z - BODY_TOP) - state.z;
    }
    const roughRise = headroom < 0.30 ? 0 : clamp(nat.rough / 0.25, 0, 1) * 0.22;
    const height = state.height * 0.92 * (1 + roughRise) * (1 - 0.16 * nat.pump);
    ...
    Y.LEGS.forEach(function (L) {
      ...
      /* Signé, et non en valeur absolue : une patte se lève pour MONTER. Une
         marche qui descend, la roue la quitte toute seule — la suspension se
         détend, c'est son métier. En levant aussi pour descendre, les pattes
         avant se relançaient sans fin au bord de l'appui d'une fenêtre (elles
         voient le vide 220 mm devant), les deux places de franchissement
         restaient prises, et les pattes arrière n'obtenaient jamais la leur :
         le robot s'arrêtait au milieu de la fenêtre. */
      const jump = ja[0], spread = Math.abs(ja[1]);
      if (!nat.wstep[L.id] && jump > WHEEL_R * 0.9 && jump <= WHEEL_CLIMB &&
        ...
        nat.wfloor[L.id] = (cur === null || cur === undefined) ? raw : cur;
        stepping++;
      }
      ...
      if (nat.wheelAir) {
        ...
        /* Et jamais sous le sol qui est là MAINTENANT. La marche visée est à
           220 mm devant ; quand elle est plus basse que l'endroit d'où l'on
           part — le bord d'un deck, le haut d'une transition —, l'arc plongeait
           droit dans le béton qu'on n'a pas encore quitté : 260 mm de roue
           dedans. Une patte qui descend une marche reste sur l'arête tant
           qu'elle ne l'a pas dépassée.
           Ce plancher est borné en vitesse, et pas pris brut sur le sol : le
           sol saute d'un coup au bord d'une marche, et un `max` avec lui
           faisait sauter la roue avec — 135 rad/s au genou dans un escalier.
           Une patte ne traverse pas le béton, et elle ne se téléporte pas non
           plus : c'est la même discipline que la suspension. */
        const fl0 = nat.wfloor[L.id];
        const rateF = Math.min(0.55 + Math.abs(nat.vx) * 1.2, 3.0) * dt;
        const floor = (fl0 === null || fl0 === undefined) ? raw
          : clamp(raw, fl0 - rateF, fl0 + rateF);
        nat.wfloor[L.id] = floor;
        h = Math.max(h, floor);
        contact = s > 0.85;
  ...
  function stepWheelFigure(dt, state) {
    ...
    if (f.kind === "tilt") {
      ...
      /* --- vrille conservée --- La pirouette passe le relais à la tenue : le
         robot bascule sur deux roues et continue de tourner sur lui-même.
         C'est une vitesse, pas un angle paramétré : elle se tient tant qu'on
         garde les épaules, et elle se freine proprement quand on lâche. */
      const wantW = run.twirlHold
        ? (run.twirlDir || 1) * Math.min(run.twirlCap || TWIRL_MAX, TWIRL_MAX)
        : 0;
      ...
      base += run.hopZ;                            // le saut soulève tout l'ensemble
      ...
      if (t < t1) {                                 // charge sur l'appui
        ...
        /* Fondu d'entrée. On n'arrive pas toujours du repos : enchaîner une
           pirouette sur une tenue, c'est passer d'une caisse gîtée et de
           pattes en pose de vrille à un appui à plat. Sans fondu, ce passage
           coûtait 22 rad/s en une image — la moitié de plus que la butée. */
        const e = smooth(clamp(t / 0.18, 0, 1));
        state.pitch = lerp(run.entryPitch, 0, e);
        state.roll = lerp(run.entryRoll, 0, e);
        place(function (L, h) { return h + WHEEL_R; });
        if (e < 1) {
          Y.LEGS.forEach(function (L, li) {
            const n = Y.Robot.legs[L.id];
            assign(n, [0, 1, 2].map(function (i) {
              return lerp(run.entryQ[li * 3 + i], n.q[i], e);
            }));
          });
        }
      } else {
      ...
      if (t < t1) {
        ...
        /* Signe compris : `nat.wz` est la vitesse de lacet RÉELLE, celle que
           lit la couche roues pour faire tourner les pneus et régler
           l'allure. En la laissant positive quel que soit le sens, une
           pirouette à droite faisait rouler les roues comme pour un virage à
           gauche — d'où des transitions propres d'un côté et fausses de
           l'autre. */
        nat.wz = run.spinW * (run.spinDir || 1);
        const s = clamp(run.spinW / wMax, 0, 1);
        state.roll = f.lean * s * (run.spinDir || 1);
        run.exitRoll = state.roll;                  // d'où repart la stabilisation
        state.z = base + ride * (1 - 0.10 * s) + WHEEL_R;
        ...
        state.yaw = run.yaw0 + run.spinA * (run.spinDir || 1);
        // on repart de la gîte où la vrille s'est arrêtée, pas d'une valeur
        // fixe : sinon la caisse sautait de deux degrés au raccord
        state.roll = lerp(run.exitRoll || 0, 0, s);
      ...
      if (t < t1) {
        ...
        phase = "réception";
        /* Les roues touchent : la vitesse emportée n'a plus lieu d'être, le
           robot suit de nouveau son cap. La garder jusqu'à la fin de la
           figure faisait glisser la réception en travers — la caisse tournait
           mais la trajectoire restait celle du décollage. Un slide, lui, vit
           de cette glissade : on ne la lui retire pas. */
        if (f.kind !== "slide") run.carry = null;
        ...
        phase = "stabilisation";
        nat.wz = approach(nat.wz, clamp(state.wz, -1.6, 1.6), 3.0, dt);
        /* Et on l'APPLIQUE ici aussi. La réception le faisait, la
           stabilisation non : la correction s'accumulait sans rien changer,
           puis tombait d'un coup à la fin de la figure — cinq degrés de
           saut, et un cap qui ne répondait plus pendant la moitié de la
           réception. C'est le décalage qu'on sentait en sortie de 180. */
        if (f.twist || f.spinTurns) {
          run.freeYaw += nat.wz * dt;
          state.yaw = run.yaw0 + 2 * Math.PI * (f.spinTurns || f.twist) + run.freeYaw;
        } else state.yaw += nat.wz * dt;
    ...
    if (run.t >= f.duration) {
      ...
      // `freeYaw` compris : c'est la correction de cap donnée par le pilote
      // pendant la réception. L'oublier ici rendait vingt-trois degrés d'un
      // coup à la dernière image de la figure.
      if (f.twist) state.yaw = run.yaw0 + 2 * Math.PI * f.twist + run.freeYaw;
      if (f.spinTurns) state.yaw = run.yaw0 + 2 * Math.PI * f.spinTurns + run.freeYaw;
  ...
  Y.Stunt = {
    ...
    start: function (name, charge, hold) {
      ...
      run.entryRoll = Y.Motion.state.roll; run.entryPitch = Y.Motion.state.pitch;
      ...
      run.twirlW = 0; run.twirlHold = false; run.twirlDir = 1; run.twirlCap = 0;
      run.hop = null; run.hopZ = 0; run.hopArmed = false; run.hopCharge = 0;
      run.exitRoll = 0;
     ...
     */
    /**
     * Passer d'une pirouette à une tenue SANS cesser de tourner.
     *
     * Les trois figures ne se disputaient qu'un seul emplacement : demander
     * un cabrage pendant une vrille arrêtait la vrille. Elles n'agissent
     * pourtant pas sur les mêmes choses — la vrille sur le lacet, la tenue
     * sur l'assiette et l'appui, le saut sur la hauteur. La tenue devient
     * donc la figure, et la vrille son modificateur : on lui passe la vitesse
     * de rotation du moment, et elle continue.
     */
    chain: function (id) {
      const f = FIGURES[id];
      if (!f || f.kind !== "tilt") return false;
      if (!run.fig || run.fig.kind !== "spin") return false;
      const w = run.spinW * (run.spinDir || 1);
      const ok = this.start(id);
      if (!ok || ok === "pente") return ok;
      run.twirlW = w;
      run.twirlDir = w < 0 ? -1 : 1;
      run.twirlCap = Math.min(Math.abs(w), TWIRL_MAX);
      run.twirlHold = true;
      return ok;
    },
    /**
     * Lancer la vrille depuis une tenue déjà en cours.
     *
     * L'enchaînement marche donc dans les deux sens : on peut partir d'une
     * pirouette et basculer, ou se dresser d'abord et se mettre à tourner
     * ensuite. Le sens alterne comme celui d'une pirouette libre.
     */
    twirlStart: function () {
      if (!run.fig || run.fig.kind !== "tilt" || run.twirlHold) return false;
      if (run.t < run.fig.arm) return false;             // pas avant d'être dressé
      run.twirlDir = spinTurn; spinTurn = -spinTurn;
      run.twirlCap = TWIRL_MAX;
      run.twirlHold = true;
      return true;
    },
    /** Vrille conservée : en cours ? */
    twirling: function () { return !!run.fig && run.fig.kind === "tilt" && Math.abs(run.twirlW) > 0.15; },
    /** Lâcher la vrille conservée : elle freine et s'arrête. */
    twirlRelease: function () { run.twirlHold = false; },

    /**
     * Sauter SUR PLACE depuis une tenue, sans quitter la position.
     *
     * Une détente verticale courte : le robot part cabré ou sur le flanc, et
     * y retombe. C'est le troisième étage de l'enchaînement — vrille, tenue,
    ...
    /** Détendre le saut en position. La hauteur vaut ce qu'on a chargé. */
    hopFire: function () {
      if (!run.hopArmed) return false;
      const k = clamp(run.hopCharge / HOP_CHARGE, 0, 1);
      run.hopArmed = false;
      run.hop = { t: 0, v: HOP_VZ * (0.55 + 0.45 * k) };
      return true;
    },
    /** Saut en position armé, en attente de détente ? */
    hopArming: function () { return !!run.hopArmed; },
    /** Saut sur place en cours ? */
    hopping: function () { return !!run.hop; },
