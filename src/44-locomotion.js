...
(function (Y) {
  ...
  function stepWheelFigure(dt, state) {
    ...
    if (f.kind === "tilt") {
      ...
      const wantW = run.twirlHold
        ? Math.sign(run.twirlW || 1) * Math.min(Math.abs(run.twirlW) || TWIRL_MAX, TWIRL_MAX)
        : 0;
  ...
  Y.Stunt = {
    ...
    start: function (name, charge, hold) {
      ...
      run.twirlW = 0; run.twirlHold = false; run.hop = null; run.hopZ = 0;
     ...
     */
    hop: function () {
      if (!run.fig || run.fig.kind !== "tilt" || run.hop) return false;
      if (run.t < run.fig.arm) return false;             // pas avant d'être dressé
      run.hop = { t: 0, v: HOP_VZ };
      return true;
    },
