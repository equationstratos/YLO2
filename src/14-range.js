...
(function (Y) {
  ...
  const G = 9.81;
  /* Rayon antipersonnel du souffle, plus court que le rayon structurel : une
     silhouette debout à deux mètres de l'impact ne bouge pas. */
  const BLAST_MEN = 1.8;
  /* Une carrosserie ne saute pas au premier impact, mais elle finit par
     sauter : réservoir percé, puis feu. Quarante balles au but — une bonne
     dizaine de secondes de tir soutenu, ce qui est long quand on tient une
     détente. */
  const CAR_HP = 40;
  ...
  function reset() {
    ...
    S.carHP = CAR_HP;
    /* On réarme le franchissement : une remise à zéro faite alors qu'on se
       trouve DÉJÀ sur la ligne doit relancer une série, sinon il faudrait
       ressortir pour un geste qui vient justement de tout remettre à neuf. */
    S.wasIn = false;
  ...
  function raise() {
    /* Nouvelle série, terrain neuf : on remet d'aplomb ce qu'on a démoli, et
       on relève ensuite. La réparation ne touche que les volumes affichés et
       la description qui sert au contact — elle ne repasse pas par la mise en
       place du stand, donc les silhouettes en place restent les mêmes. */
  ...
  /**
   * Tir LIBRE : sans verrou, le coup part droit devant.
   *
   * La visée automatique choisit toujours la plus proche à découvert et dans
   * son cône ; tout le reste — une silhouette hors cône, plus loin que la
   * portée de recherche, ou qu'on veut simplement prendre soi-même — n'était
   * pas atteignable. Refuser le coup rendait la détente muette, tirer sans
   * rien calculer la rendait décorative.
   *
   * On lance donc un vrai rayon : direction du canon, écartée de la
   * dispersion, et la première silhouette debout qui se trouve dessus tombe.
   * Le mur compte, comme toujours : ce qu'il arrête n'atteint rien derrière.
   */
  const qBar = new T.Quaternion(), vDir = new T.Vector3();

  function freeShot(from, spread) {
    S.barrel.getWorldQuaternion(qBar);
    vDir.set(1, 0, 0).applyQuaternion(qBar);
    // la dispersion écarte le rayon : deux angles, comme pour une vraie arme
    const ax = randn() * spread, az = randn() * spread;
    let yaw = Math.atan2(vDir.y, vDir.x) + az;
    let pit = Math.asin(Math.max(-1, Math.min(1, vDir.z))) + ax;
    const d = new T.Vector3(Math.cos(pit) * Math.cos(yaw),
                            Math.cos(pit) * Math.sin(yaw), Math.sin(pit));
    const reach = W().reach;
    const wall = Y.Terrain.hitDist(from.x, from.y, from.z,
      from.x + d.x * reach, from.y + d.y * reach, from.z + d.z * reach, MUZZLE_SKIP);
    const stop = wall < 0 ? reach : wall;
    let best = null, bd = Infinity;
    S.targets.forEach(function (t) {
      if (t.friend || t.state !== "up") return;
      /* Distance du CENTRE de la silhouette au rayon, par projection. Une
         cible n'est un point ni en hauteur ni en largeur : on tolère sa
         demi-largeur autour de l'axe, et on vérifie qu'on la prend entre ses
         pieds et sa tête. */
      const ox = t.x - from.x, oy = t.y - from.y, oz = aimZ(t) - from.z;
      const along = ox * d.x + oy * d.y + oz * d.z;
      if (along <= 0.3 || along > stop || along > bd) return;
      const px = ox - d.x * along, py = oy - d.y * along, pz = oz - d.z * along;
      if (Math.hypot(px, py) > TARGET_W * 0.5) return;
      if (Math.abs(pz) > TARGET_H * 0.45) return;
      bd = along; best = t;
    });
    return { t: best, end: new T.Vector3(from.x + d.x * (best ? bd : stop),
                                         from.y + d.y * (best ? bd : stop),
                                         from.z + d.z * (best ? bd : stop)) };
  }

  /**
   * Une balle a-t-elle frappé la carcasse — et celle-ci en a-t-elle assez ?
   *
   * Le lance-grenades n'est pas la seule façon de faire sauter une voiture :
   * une rafale soutenue y arrive aussi, plus lentement. On regarde donc où la
   * balle s'est arrêtée, et si c'est dans la carcasse, on décompte. Le point
   * d'arrêt est déjà calculé pour le traceur : il n'y a rien à lancer de
   * plus, c'est la même géométrie qui sert à dessiner et à compter.
   */
  function hitsCar(pt) {
    const boxes = Y.Terrain.current.boxes;
    for (let i = 0; i < boxes.length; i++) {
      const b = boxes[i];
      if (b.part !== "auto") continue;
      const m = 0.12;                       // on tolère l'épaisseur du traceur
      if (pt.x >= b.x0 - m && pt.x <= b.x1 + m &&
          pt.y >= b.y0 - m && pt.y <= b.y1 + m &&
          pt.z >= (b.z0 || 0) - m && pt.z <= b.h + m) return true;
    }
    return false;
  }

  function damageCar(pt) {
    if (S.wrecked) return;
    S.carHP--;
    if (S.carHP > 0) {
      // un impact sur tôle, et l'état du véhicule qu'on peut lire au HUD
      Y.Audio.hit();
      S.say = "Véhicule touché — " + Math.round(S.carHP / CAR_HP * 100) + " %";
      return;
    }
    /* Le réservoir lâche. L'explosion est la même que celle d'une grenade —
       c'est le véhicule qui saute, pas ce qui l'a percé. */
    explode(pt.clone());
  }
  ...
  function shoot() {
    ...
    /* Pas de verrou : on tire quand même, et le rayon décide. C'est le seul
       chemin par lequel une cible hors du cycle de visée peut tomber. */
    let free = null;
    if (!t) {
      const speed = Math.abs(Y.Natural.state.vx);
      const sp = (W().spread0 + speed * W().spreadV
                  + (W().burst - S.burst) * W().recoil
                  + S.gimbalErr * 180 / Math.PI * 0.20) * Math.PI / 180;
      free = freeShot(v0, sp);
      if (free.t) {
    ...
    // et si ce mur-là était une carrosserie, elle vient d'en prendre une
    if (!hit && (!free || !free.t) && hitsCar(v1)) damageCar(v1);
  }
  ...
  function explode(p) {
    ...
    /* Le souffle a DEUX rayons, et c'est ce qui manquait.
       Une grenade de 40 mm démolit une carrosserie à trois mètres mais ne
       couche pas une silhouette de contreplaqué à la même distance ; les
       cibles du stand étant espacées de deux mètres cinquante, un rayon
       unique de 3,4 m en emportait trois ou quatre d'un coup et l'on voyait
       tomber des cibles que rien n'avait touchées. Le rayon ANTIPERSONNEL
       est donc plus court que le rayon structurel : ce qui est vraiment dans
       le souffle tombe, le reste reste debout. */
    S.targets.forEach(function (t) {
      if (t.friend || t.state !== "up") return;
      if (Math.hypot(t.x - p.x, t.y - p.y, aimZ(t) - p.z) > BLAST_MEN) return;
  ...
  function step(dt) {
    ...
    /* La zone verte relance une série NEUVE, et elle le fait au
       ...
       Trois cas se comportaient différemment, et c'est ce qui rendait la
       reprise imprévisible : une série finie repartait toute seule sans qu'on
       bouge, une série abandonnée en cours de route ne repartait jamais (ni
       relève ni remise à zéro : les cibles restaient comme on les avait
       laissées), et une entrée normale relevait tout. Un seul cas
